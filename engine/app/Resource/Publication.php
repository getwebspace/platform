...
class Publication extends \AbstractResource
{
    public function count(array $criteria = [])
    {
        return $this->entityManager->getRepository(\Entity\Publication::class)
                    ->count($criteria);
    }
    ...
    {
        return collect(
            $this->entityManager->getRepository(\Entity\Publication::class)
    ...
    public function search(array $criteria = [], array $orderBy = [])
    {
        $query = $this->entityManager
                      ->getRepository(\Entity\Publication::class)
        ...
        foreach ($orderBy as $field => $direction) {
            $query->orderBy("p.{$field}", $direction);
        }
    ...
    public function flush(array $data = [])
    {
        ...
        $publication = $this->fetchOne(['uuid' => $data['uuid']]) ?? new \Entity\Publication();
        $publication->replace($data);
