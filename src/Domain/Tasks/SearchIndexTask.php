...
class SearchIndexTask extends AbstractTask
{
    ...
    protected function action(array $args = []): void
    {
        ...
        foreach ($pageService->read() as $item) {
            /** @var Page $item */
            $index[] = $this->implode('page', $item->getUuid(), Search::getIndexedText([
                $item->getTitle(),
                $item->getContent(),
                $item->getMeta()['title'],
        ...
        foreach ($publicationService->read() as $item) {
            ...
            $index[] = $this->implode('publication', $item->getUuid(), Search::getIndexedText([
                $item->getTitle(),
    ...
    protected function implode(string $type, UuidInterface $uuid, array $data): string
    {
