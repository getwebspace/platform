...
class Whois
{
    private static array $whoisServers = [
    ...
    public static function query($domain, $type = 'array', $raw = false)
    {
        static::$returnType = $type;
        ...
        if ($explodedCount >= 3) {
            if (array_key_exists($explodedDomain[$explodedCount - 1], static::$whoisServers)) {
                if (array_key_exists($explodedDomain[$explodedCount - 2], static::$whoisServers)) {
        ...
        $parsedResult = static::parse_domain($data, $domain, $tld);
    ...
    private static function getWhoisResult($domain = '', $tld = '', $serverUrl = '')
    {
        if (empty($domain)) {
        ...
        if (!array_key_exists($tld, static::$whoisServers)) {
            return ['++[ERROR_UNSUPPORTED_TLD]++'];
        }

        if (empty($serverUrl)) {
            $serverUrl = static::$whoisServers[$tld];
        }
        $f = fsockopen($serverUrl, 43);

        $needToBeAddEqual = ['whois.verisign-grs.com', 'whois.verisign-grs.net'];

        if (in_array($serverUrl, $needToBeAddEqual, true)) {
            fwrite($f, '=' . $domain . "\r\n");
        } else {
            fwrite($f, $domain . "\r\n");
        }
        ...
        if (static::$returnType !== 'text') {
            ...
            foreach ($data as $eachLine) {
                foreach ($stringsExistToMeanAvailable as $stringExistToMeanAvailable) {
                    if (mb_strpos($eachLine, $stringExistToMeanAvailable) !== false) {
                        return ['++[ERROR_NO_MATCH]++'];
                    }
                }
                foreach ($stringsExistToMeanErrorInput as $stringExistToMeanErrorInput) {
                    if (mb_strpos($eachLine, $stringExistToMeanErrorInput) !== false) {
                        return ['++[ERROR_INPUT]++'];
                    }
                }
                foreach ($stringsExistToMeanWaitSomeTime as $stringExistToMeanWaitSomeTime => $sscanfFullString) {
                    if (mb_strpos($eachLine, $stringExistToMeanWaitSomeTime) !== false) {
    ...
    private static function parse($data, $domain, $domainWord, $keywords, $breakOnEnter)
    {
        $found = false;
        $domainWordLen = mb_strlen($domainWord);
        ...
        foreach ($data as $d) {
            ...
            $pos = mb_strpos($d, $domainWord);
            if ($pos !== false) {
                $dom = mb_strtolower(trim(mb_substr($d, $pos + $domainWordLen)));
                if ($dom === $domain) {
                    $found = true;
                }
            }
            ...
            if ($found) {
                $pos = mb_strpos($d, ':');
                if ($pos !== false) {
                    $keyword = mb_substr($d, 0, $pos);

                    if (isset($keywords[$keyword])) {
                        $t = trim(mb_substr($d, $pos + 1));
                        if ($t !== '') {
    ...
    private static function parse_domain($data, $domain, $tld)
    {
        $domainWords = [
            'def' => 'Domain Name:',
            'com' => 'Domain Name:',
        ...
        $data = array_filter(array_map('\App\Application\Whois::ifWhiteSpace', $data));
        $parseResult = static::parse($data, $domain, $domainWord, $toBeParseKeywords, true);
        ...
        if (!empty($parseResult['domain'])) {
            $parseResult['domain'] = mb_strtolower($parseResult['domain']);
        }
        ...
        foreach ($parseResult as $eachParseResultKey => $eachParseResult) {
            if (mb_substr($eachParseResultKey, 0, 6) === 'admin_') {
                unset($parseResult[$eachParseResultKey]);
                $key = mb_substr($eachParseResultKey, 6);
                if ($key !== 'address') {
                    $eachParseResult = $eachParseResult[0];
                }
                $parseResult['admin'][$key] = $eachParseResult;
            }
            if (mb_substr($eachParseResultKey, 0, 11) === 'registrant_') {
                unset($parseResult[$eachParseResultKey]);
                $key = mb_substr($eachParseResultKey, 11);
                if ($key !== 'address') {
                    $eachParseResult = $eachParseResult[0];
                }
                $parseResult['registrant'][$key] = $eachParseResult;
            }
            if (mb_substr($eachParseResultKey, 0, 5) === 'tech_') {
                unset($parseResult[$eachParseResultKey]);
                $key = mb_substr($eachParseResultKey, 5);
                if ($key !== 'address') {
                    $eachParseResult = $eachParseResult[0];
                }
                $parseResult['tech'][$key] = $eachParseResult;
            }
            if (mb_substr($eachParseResultKey, 0, 8) === 'billing_') {
                unset($parseResult[$eachParseResultKey]);
                $key = mb_substr($eachParseResultKey, 8);
                if ($key !== 'address') {
                    $eachParseResult = $eachParseResult[0];
                }
        ...
        foreach ($needToBeLowerCasedValueFathers as $needToBeLowerCasedValueFather) {
            if (!empty($parseResult[$needToBeLowerCasedValueFather])) {
                ...
                if (!empty($needToBeLowerCasedValue)) {
                    $parseResult[$needToBeLowerCasedValueFather]['email'] = mb_strtolower($parseResult[$needToBeLowerCasedValueFather]['email']);
                }
            }
        }
