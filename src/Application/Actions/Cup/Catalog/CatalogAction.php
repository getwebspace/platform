...
<div class="m-5">
    <div class="row">
        ...
        <div class="col-6 text-right">
            Заказ #: <b>{{ order.serial }}</b><br />
            Дата: <b>{{ order.date|df('d.m.Y H:i') }}</b>
        </div>
    </div>

    <div class="row mt-3">
        <div class="col-6 ">
            {{ parameter('common_title') }}
        </div>
        <div class="col-6 text-right">
    ...
    {% for product in products %}
        ...
        <div class="row p-1 {{ loop.last ?: 'border-bottom' }}">
            <div class="col-6">{{ product.title }}</div>
            <div class="col-2 text-center">{{ product.price|number_format(2, '.', ' ') }}</div>
